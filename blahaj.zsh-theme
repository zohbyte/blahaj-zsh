# Transgender-colored theme based on gnzh based on bira
...
() {
  ...
  if [[ -n "$SSH_CLIENT" || -n "$SSH2_CLIENT" ]]; then
    PR_HOST="${BLAHAJ_PINK}%M${BLAHAJ_RESET}"
  else
    PR_HOST="${BLAHAJ_BLUE}%m${BLAHAJ_RESET}"
  fi
...
ZSH_THEME_RUBY_PROMPT_PREFIX="%F{205}‹"
ZSH_THEME_RUBY_PROMPT_SUFFIX=">%f"
ZSH_THEME_VIRTUALENV_PREFIX="%F{205}("
